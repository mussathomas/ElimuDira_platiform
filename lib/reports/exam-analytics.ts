...
export async function loadExamAnalytics(schoolId: string, filters: ResultFilters) {
  ...
  const studentPerformance = results.map((result: AnyRow) => { const student = studentsById.get(result.student_id); return { ...result, student, className: student?.classes?.name ?? 'Unassigned' }; }).sort((left, right) => (left.position ?? Number.MAX_SAFE_INTEGER) - (right.position ?? Number.MAX_SAFE_INTEGER));
