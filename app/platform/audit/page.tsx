...
export default async function SystemAuditPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  const { data: logs, count } = await supabase
    .from('audit_logs')
    .select('id, action, resource_type, resource_id, created_at, school_id, actor_id, schools(name)', { count: 'exact' })
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-xl font-semibold text-ink">System audit log</h2>
        <p className="help-text">Every recorded action across every school — read-only, append-only.</p>
      </div>
          ...
          <tbody>
            {(logs ?? []).map((log) => (
              <tr key={log.id} className="border-b border-line last:border-0">
                ...
                <td className="px-4 py-3 text-ink-soft">{(log.schools as any)?.name ?? '\u2014 (platform)'}</td>
      ...
      <p className="help-text">
        {count ?? 0} total entries · page {page}
      </p>
    </div>
  );
}
